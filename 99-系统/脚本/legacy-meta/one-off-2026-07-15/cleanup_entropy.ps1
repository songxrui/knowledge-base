...
$mainHashes = @{}
$excludePrefixes = @("99-系统\alchemist\", ".codex\", ".dbs\", ".obsidian\", "tmp\", "90-归档/历史版本\")
...
foreach ($f in (Get-ChildItem "$vault\99-系统\alchemist" -Recurse -Filter "*.md" -File)) {
...
$temps = @("$vault\99-系统\日志\broken_links.csv", "$vault\99-系统\日志\broken_links_v2.csv", "$vault\99-系统\日志\orphan_notes.csv")
foreach ($t in $temps) {
    if (Test-Path $t) { Remove-Item $t -Force; Write-Host "Deleted: temp CSV" }
}

# Step 4: Delete empty directories in _alchemist
$emptyDirs = (Get-ChildItem "$vault\99-系统\alchemist" -Recurse -Directory | Where-Object { (Get-ChildItem $_.FullName -Recurse -File).Count -eq 0 }) | Sort-Object FullName -Descending
foreach ($d in $emptyDirs) {
    Remove-Item $d.FullName -Force
    $relativePath = $d.FullName.Replace("$vault\", "")
    Write-Host "Removed empty dir: $relativePath"
}
...
[System.IO.File]::WriteAllText("$vault\99-系统\日志\CLEANUP_REPORT_2026-06-09.md", $report, $utf8Bom)
Write-Host "`nReport saved: 99-系统\日志\CLEANUP_REPORT_2026-06-09.md" -ForegroundColor Green
