...
for root in roots:
    ...
    for name in os.listdir(root):
        ...
        # FIXED: check full content, not just first 500 chars
        g2_trigger = any(k in content.lower()[:2000] for k in ["description", "when to use"]) or \
                     any(k in content[:2000] for k in ["触发", "适用场景"])
        g2_boundary = any(k in content[:2000] for k in ["不适用", "不适合", "排除", "边界", "限制"])
...
print(f"=== Skill Quality Scan (FIXED) ===")
print(f"Total: {total}")
...
with open(r"D:\KnowledgeBase\99-系统\元数据\skill_gate_scan.json", "w", encoding="utf-8") as f:
