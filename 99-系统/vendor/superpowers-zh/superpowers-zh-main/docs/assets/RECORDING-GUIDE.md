...
```bash
# mov 转 gif（用 ffmpeg）
ffmpeg -i recording.mov -vf "fps=10,scale=700:-1:flags=lanczos" -c:v gif docs/08-媒体与产品/素材/demo.gif

# 如果太大（>2MB），降低 fps 或尺寸
ffmpeg -i recording.mov -vf "fps=8,scale=600:-1:flags=lanczos" -c:v gif docs/08-媒体与产品/素材/demo.gif
```
...
vhs docs/08-媒体与产品/素材/demo.tape
```
