...
const SERVER_PATH = path.join(__dirname, '../../skills/brainstorming/99-系统/脚本/server.cjs');
  ...
  try {
    ...
    await test('helper.js defines required APIs', () => {
      const helperContent = fs.readFileSync(
        path.join(__dirname, '../../skills/brainstorming/99-系统/脚本/helper.js'), 'utf-8'
      );
    ...
    await test('frame template has required structure', () => {
      const template = fs.readFileSync(
        path.join(__dirname, '../../skills/brainstorming/99-系统/脚本/frame-template.html'), 'utf-8'
      );
