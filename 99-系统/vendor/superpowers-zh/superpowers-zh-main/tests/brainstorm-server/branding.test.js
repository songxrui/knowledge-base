...
const SERVER_PATH = path.join(REPO_ROOT, 'skills/brainstorming/99-系统/脚本/server.cjs');
...
function assertFramedLogoSupportsDarkTheme(html) {
  assert(
    /@08-媒体与产品\媒体\s*\(prefers-color-scheme:\s*dark\)[\s\S]*\.brand-logo\s*\{[^}]*filter:\s*none/i.test(html),
