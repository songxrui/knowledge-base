...
```bash
python 99-系统/脚本/migrate.py --verify --backup
```
...
运行：`python 99-系统/脚本/analyze_form.py input.pdf`
...
运行：`python 99-系统/脚本/validate_fields.py fields.json`

在继续之前修复所有验证错误。

**步骤 4：填写表单**

运行：`python 99-系统/脚本/fill_form.py input.pdf fields.json output.pdf`

**步骤 5：验证输出**

运行：`python 99-系统/脚本/verify_output.py output.pdf`
...
2. **立即验证**：`python ooxml/99-系统/脚本/validate.py unpacked_dir/`
3. 如果验证失败：
   - 仔细审查错误信息
   - 修复 XML 中的问题
   - 再次运行验证
4. **只有验证通过后才继续**
5. 重新打包：`python ooxml/99-系统/脚本/pack.py unpacked_dir/ output.docx`
6. 测试输出文档
```
...
* ✓ **好的**：`99-系统/脚本/helper.py`、`reference/guide.md`
* ✗ **避免**：`99-系统\脚本\helper.py`、`reference\guide.md`
...
```bash
python 99-系统/脚本/analyze_form.py input.pdf > fields.json
```
...
```bash
python 99-系统/脚本/validate_boxes.py fields.json
# 返回："OK" 或列出冲突
```
...
```bash
python 99-系统/脚本/fill_form.py input.pdf fields.json output.pdf
```
````
...
1. 将 PDF 转换为图像：
   ```bash
   python 99-系统/脚本/pdf_to_images.py form.pdf
   ```
