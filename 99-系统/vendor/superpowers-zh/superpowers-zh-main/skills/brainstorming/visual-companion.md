...
```bash
# 启动服务器并持久化（原型保存到项目中）
99-系统/脚本/start-server.sh --project-dir /path/to/project
...
# 让进程在轮次之间存活
99-系统/脚本/start-server.sh --project-dir /path/to/project --foreground
```
...
```bash
99-系统/脚本/stop-server.sh $SCREEN_DIR
```
...
- 框架模板（CSS 参考）：`99-系统/脚本/frame-template.html`
- 辅助脚本（客户端）：`99-系统/脚本/helper.js`
