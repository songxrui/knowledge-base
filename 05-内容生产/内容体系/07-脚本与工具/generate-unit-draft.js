...
/**
 * Truncate title to maxLen characters, preserving full Unicode characters.
 * Appends "…" if truncated. Default maxLen = 40.
 */
function truncateTitle(title, maxLen = 40) {
  if (title.length <= maxLen) return title;
  // Convert to array of code points to avoid splitting surrogate pairs or multi-byte chars
  const chars = [...title];
...
const fileName = `${id}_${shortTitle}.md`;
const targetPath = path.join(targetDir, fileName);

if (shortTitle !== title) {
  console.warn(`[警告] 标题过长 (${title.length} 字符)，已截断为 ${shortTitle.length} 字符: "${shortTitle}"`);
}
