...
| **GitHub Token** | `<GITHUB_TOKEN>`（完全权限，通过 `gh` CLI 操作） |
| **飞书 CLI** | `D:\npm-global\feishu-codex-bridge.ps1`（已安装，npm global 路径） |
| **飞书空间** | `jcn1crrvstv9` |
| **飞书加密文件夹** | `<FEISHU_FOLDER_URL>`（密码: `<FEISHU_FOLDER_PASSWORD>`，内含 dontbesilent 付费社群笔记） |
| **微信读书 API** | `<WEREAD_API_URL>`，Token: `wrk-yC_PeQeCQBWIBD7_uFhTwwAA`，skill_version: `1.0.3` |
| **Exa Search MCP** | Token: `<EXA_API_KEY>`（书籍内容优先） |
...
├── 08-媒体与产品/素材/            # 媒体资产
├── 04-知识/卡片/             # 35 张深度内容卡片
├── 08-媒体与产品/媒体/             # 多平台媒体稿（公众号127篇、即刻、twitter、小红书、知乎、newsletter）
├── notion/            # Notion 导出（hui2737，155个MD）
├── output/            # 产出交付（85个MD文件，含报告、日志、prompts）
├── 99-系统/脚本/           # 自动化脚本
├── 03-资源/来源台账/           # 原始信源素材
├── 04-知识/原子笔记/            # 11 条原子笔记（概念导向、双链）
├── zhanshimian/       # 展示面素材
├── 选题管理/          # 选题规划
├── 99-系统/alchemist/        # 内容炼金工程（674个MD，109个子目录）
├── _archive/          # 历史归档
├── 90-归档/旧内容系统入口/   # 结构化内容系统
├── _deprecated/       # 已废弃
├── 99-系统/日志/             # 操作日志
└── .codex/            # Codex 配置
```

**_alchemist 子工程**（`D:\KnowledgeBase\99-系统\alchemist\`）：
```
99-系统/alchemist/
├── 04-知识/卡片/             # 35 张 9.5 分深度卡（来自 7 个母题簇）
...
├── strategy/          # 策略文档
├── 04-知识/原子笔记/            # 原子笔记副本
├── 90-归档/旧内容系统入口/   # dbs-content-system 工程输出
├── 99-系统/日志/             # 全程日志（TOOL_LEDGER.md, weread_raw.json, weread_books_full.txt 等）
└── audit-trail/       # 审计追踪
```
...
- **已有缓存**: `D:\KnowledgeBase\99-系统\alchemist\99-系统\日志\weread_raw.json`（272KB）、`weread_books_full.txt`（70行）
- **提取要求**: 拉 >=30 条划线/想法，贴入 `D:\KnowledgeBase\output\99-系统\日志\SOURCE_PROOF.md`（验证真实调用）
...
D:\npm-global\feishu-codex-bridge.ps1 -Action "read_doc" -DocUrl "<FEISHU_FOLDER_URL>" -Password "<FEISHU_FOLDER_PASSWORD>"
```

### Step 2【Skill 强制调用】
选择 >=1 个内容创作 skill 对该条目执行改写/钩子提炼/密度提升，记入 `TOOL_LEDGER`。

**TOOL_LEDGER 格式**（`D:\KnowledgeBase\output\99-系统\日志\TOOL_LEDGER.md`）:
```
...
### Step 5【质量复审】
重新打分，<9.0 → 返回 Step 1 重做（记入 `RETRY_LOG`，路径: `D:\KnowledgeBase\output\99-系统\日志\RETRY_LOG.md`）。
...
| 文件 | 完整路径 | 内容 |
|------|---------|------|
| **TOOL_LEDGER** | `D:\KnowledgeBase\output\99-系统\日志\TOOL_LEDGER.md` | 每次 skill 调用一行（时间/skill/输入摘要/输出摘要/是否有效/用于哪条 ID） |
| **EXECUTION_LOG** | `D:\KnowledgeBase\output\99-系统\日志\EXECUTION_LOG.md` | 每次 REJECTED 记原因 + 触发哪条红线 + 重写次数 |
| **RETRY_LOG** | `D:\KnowledgeBase\output\99-系统\日志\RETRY_LOG.md` | 每次 Phase 2 返工记录（哪条ID/第几次/改了什么） |
| **SOURCE_PROOF** | `D:\KnowledgeBase\output\99-系统\日志\SOURCE_PROOF.md` | 微信读书/Notion/飞书/AI 会话真实接通证明（样例 >=30 行） |
| **BLOCKED_CMDS** | `D:\KnowledgeBase\output\99-系统\日志\BLOCKED_CMDS.md` | 被阻塞的命令及替代方案 |
...
# 微信读书 API 测试
$env:WEREAD_API_KEY="<SET_IN_LOCAL_ENV>"
Invoke-RestMethod -Uri "<WEREAD_API_URL>" -Method Post -Headers @{Authorization="Bearer $env:WEREAD_API_KEY"} -ContentType "application/json" -Body '{"api_name":"/_list","skill_version":"1.0.3"}'
```
